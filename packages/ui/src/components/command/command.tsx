...
>(({ className, ...props }, ref) => (
  <CommandPrimitive
    ref={ref}
    className={cn('bg-base-white flex h-full w-full flex-col overflow-hidden rounded-md text-gray-700', className)}
    {...props}
  />
));
...
const CommandInput = React.forwardRef<React.ElementRef<typeof CommandPrimitive.Input>, CommandInputProps>(
  ({ className, wrapperClassName, icon, ...props }, ref) => (
    <div className={cn('flex items-center px-3', wrapperClassName)} cmdk-input-wrapper="">
      {icon ?? <SearchIcon className="mr-2 shrink-0" />}
      <CommandPrimitive.Input
        ref={ref}
        className={cn(
          'flex h-full bg-transparent outline-none placeholder:text-gray-500 disabled:cursor-not-allowed',
          className
        )}
        {...props}
      />
